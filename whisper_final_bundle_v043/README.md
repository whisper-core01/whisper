# Whisper Remote Nerve MVP Bundle v0.4.3
...
Vault v0.1.0
VaultDisk v0.1.0
Pipeline Demo v0.1.0
MCEHardened v0.1.1
Loader v0.1.0
BAL v0.1.0
Dome v0.1.0
VoxMesh v0.1.0
Bundle README v0.3.1
```
...
PipelineDemo = end-to-end MVP flow
```
...
disk = VaultDisk(Path("vault.json"))

disk.save(vault)
loaded_vault = disk.load()

assert loaded_vault.dump() == vault.dump()
```

VaultDisk behavior:

```text
1. persists Vault metadata to JSON;
2. writes deterministic entries sorted by fragment_id;
3. uses temp-file + os.replace() for simple atomic replacement;
4. reloads entries into a fresh Vault instance;
5. does not store fragments, seeds, plaintext, ciphertext, or key material.
```

VaultDisk is disk persistence only. It is not encrypted storage, not tamper-resistant storage, not secure audit logging, and not rollback-protected.
...
```bash
python pipeline_demo.py --random-bytes 4096 --fragment-size 256 --json
```
...
The demo validates the integration shape only. It is not encryption and not a production Whisper pipeline.
...
`mce_hardened_v01.py` adds validation and coherence checks around MCE.
...
validate_state()
coherence_check()
digest_fragment_checked()
```

Scope:

```text
state hash must be 32 bytes
fragment_counter must be int >= 0
rotor count must be valid
safe checked digest wrapper
```

Benchmark:

```bash
python bench/bench_mce_hardened.py --fragments 10000
```

Target:

```text
< 10% overhead vs MCE v0.1.0
```

### Loader v0.1.0

`loader_v01.py` adds deterministic operational decisions.

Features:

```text
decide_fragment_size(payload_size)
decide_route_count(mce_state_hex)
decide_retry_policy()
decide_all(payload_size)
```

Scope:

```text
deterministic fragmentation strategy
simple route count decision, 1-3 routes
...
```bash
python bench/bench_loader.py --payloads 10000
```
...
```bash
pytest -q tests/test_week1_integration.py
```

Integration flow:

```text
payload -> Loader.decide_fragment_size()
        -> fragment_payload()
        -> MCEHardened.digest_fragment_checked()
        -> coherence_check()
```

## Week 2/3 roadmap notes

Planned next components:

```text
Week 2:
  BAL v0.1.0  = adaptive transport skeleton
  Dome v0.1.0 = filtering + envelope layer

Week 3:
  Lemonade v0.1.0 = immune system skeleton
  VoxMesh v0.1.0  = 36-fractal MVP skeleton
```

Rule:

```text
1 component = 1 file, 50-200 LOC, 3-5 tests, 1 benchmark/smoke script.
```
...
python bench/bench_vaultdisk.py --entries 10000
python bench/bench_pipeline.py --payload-size 1048576 --fragment-size 256
```


## Running benchmark scripts from outside the bundle

Benchmark scripts prepend the project root to `sys.path`, so they can be launched either from the bundle root:

```bash
cd rotor_mce_vaultdisk_pipeline_bundle_v018
python3 bench/bench_vaultdisk.py --entries 10000
```

or from the parent directory:

```bash
python3 rotor_mce_vaultdisk_pipeline_bundle_v018/bench/bench_vaultdisk.py --entries 10000
```
...
VaultDisk does not guarantee:

```text
encrypted storage
tamper detection
rollback protection
crash-proof durability
...
| VaultDisk save/load | ✓ JSON disk roundtrip preserves entries |
| Pipeline demo | ✓ payload fragments processed through MCE and Vault |
...
`bal_v01.py` adds a minimal parallel lane skeleton.
...
```bash
python bench/bench_bal.py --fragments 10000 --routes 3
```

Target:

```text
< 5% overhead vs direct processing
```

### Dome v0.1.0

`dome_v01.py` adds simple filtering and a reversible envelope.
...
```text
accept size 0..10 MiB
reject null byte runs > 1000 bytes
envelope = MAGIC + VERSION + metadata_len + fragment_len + metadata + fragment
```

Benchmark:

```bash
python bench/bench_dome.py --fragments 10000 --rejection-rate 0.05
```

Target:

```text
< 1 ms per wrap/unwrap
```

### VoxMesh v0.1.0

`voxmesh_v01.py` adds a 36-fractal state-divergence skeleton.
...
Fractal
VoxMesh.mutate_all()
VoxMesh.get_divergence_score()
VoxMesh.coherence_check()
```
...
```bash
python bench/bench_voxmesh.py --mutations 1000
```

Target:

```text
< 5 ms per full mutation cycle
```



## Benchmark note v0.3.1

BAL benchmark was adjusted because comparing BAL against `list(fragments)` was not meaningful: the direct path was a near-zero Python no-op, producing misleading overhead percentages.

The updated BAL benchmark reports:

```text
direct indexed median
BAL median
overhead vs indexed baseline
BAL ms/fragment
absolute pass < 0.01 ms/fragment
```

Dome benchmark was adjusted to avoid double-counting accepted fragments in rejection-rate tracking. The benchmark now calls `wrap_fragment()` directly and lets Dome perform exactly one acceptance check per fragment.
...
## Week 3 component: Lemonade v0.1.0

`lemonade_v01.py` adds a minimal immune-system skeleton.

Features:

```text
detect_overflow(queue_depth)
detect_spam(fragment_rate)
detect_poison(fragment)
detect_oversize(fragment)
detect_replay(fragment_id)
detect_entropy_drop(fragment)
detect_state_anomaly(validation_report)
scan_fragment(...)
```
...
```text
ThreatReport(threat_level=0..10, signals=[...], blocked=bool)
```
...
```bash
python bench/bench_lemonade.py --fragments 10000 --bad-rate 0.05
```
...
## Lemonade v0.1.1 note

`scan_fragment()` is cumulative and updates global `threat_level`.

For benchmarking or per-fragment decisions, use:

```python
report = lemonade.scan_fragment_stateless(...)
```

This returns a per-fragment `ThreatReport` without permanently poisoning the global threat state. Replay memory is still preserved because replay detection is inherently stateful.
...
Dedicated non-regression suite:
